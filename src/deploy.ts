/**
 * Deploy the Canary contract to a Midnight network (undeployed by default; use
...
// Identifier under which this contract's private state is stored. Canary's
// private state holds the responder's secret key and their wellbeing score —
// neither of which ever reaches the chain.
const PRIVATE_STATE_ID = 'canaryPrivateState';

// The deployer becomes the contract admin (the only party who may close a
// round). Derive that secret deterministically from the wallet seed so the
// admin role survives a re-deploy and is recoverable from the seed alone —
// rather than a random key that would be lost the moment this process exits.
...
// The constructor only reads localSecretKey(); the score is unused at deploy
// time but the private-state shape requires it.
const DEPLOY_PLACEHOLDER_SCORE = 3;
...
// The module is loaded at runtime (it only exists after `npm run compile`),
// but typed statically so `withWitnesses` can check the witness object against
// the contract's generated Witnesses type instead of collapsing to `never`.
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log(`║  Deploy canary to ${network}`);
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      // initialPrivateState. For Canary that is the deployer's secret key and
      // a placeholder score — both stay local; the constructor writes only
      // hash(secret) to the ledger as `admin`. args is the contract
      // constructor's arguments: empty for Canary's no-arg constructor.
      ...
      deployed = await deployContract(providers, {
        ...
        initialPrivateState: createCanaryPrivateState(
          deriveAdminSecret(seed),
          DEPLOY_PLACEHOLDER_SCORE,
        ),
      });
      break;
    } catch (err: any) {
